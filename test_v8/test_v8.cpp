...
void AccessorGetterCallbackFunction(Local<Name> property,
  ...
  String::Utf8Value utf8_value(info.GetIsolate(),
                               Local<String>::Cast(property));
  accessed_property = *utf8_value;
  if (accessed_property == "x")
    info.GetReturnValue().Set(
        v8::Integer::New(info.GetIsolate(), pointPtr->x_));
  else if (accessed_property == "y")
    info.GetReturnValue().Set(
        v8::Integer::New(info.GetIsolate(), pointPtr->y_));
}
...
int main(int argc, char* argv[]) {
  ...
  auto isolate = v8::Isolate::New(create_params);
  {
    ...
    // Set the function template function to be called when user uses Point
    // keyword
    globalObj->Set(
        context,
        v8::String::NewFromUtf8(isolate, u8"Point", v8::NewStringType::kNormal)
            .ToLocalChecked(),
    ...
    v8::Local<v8::Function> jsMainFunc =
        v8::Local<v8::Function>::Cast(jsValMainFunc);
